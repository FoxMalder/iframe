...
$(document).ready(function () {
  $('#new-keyword').keyup(function () {
    console.log('fgdfg');
    if ($(this).val().length != 0) $('.add-word .i-plus-in-circle').addClass('blue');else $('.add-word .i-plus-in-circle').removeClass('blue');
  });
});
/*
...
!function (modules) {
        ...
        inputFallBackEvent: function inputFallBackEvent(e) {
          ...
          if (getBuffer().join("") !== inputValue) {
            ...
            if (inputValue = function (input, inputValue, caretPos) {
              ...
              return inputValue;
            }(input, inputValue, caretPos), caretPos.begin > inputValue.length && (caret(input, inputValue.length), caretPos = caret(input)), getBuffer().join("") !== inputValue) {
  ...
  !function (factory) {
    ...
    function parse(format) {
      for (var match, mask = ""; match = getTokenizer().exec(format);) {
    ...
    function analyseMask(maskString, format, opts) {
      ...
      for (var targetProp, match, dateObj = {
        ...
        minutes: 0,
        seconds: 0
      }, mask = maskString; match = getTokenizer().exec(format);) {
        if ("d" === match[0].charAt(0)) targetProp = "day";else if ("m" === match[0].charAt(0)) targetProp = "month";else if ("y" === match[0].charAt(0)) targetProp = "year";else if ("h" === match[0].charAt(0).toLowerCase()) targetProp = "hour";else if ("M" === match[0].charAt(0)) targetProp = "minutes";else if ("s" === match[0].charAt(0)) targetProp = "seconds";else if (formatCode.hasOwnProperty(match[0])) targetProp = "unmatched";else {
          var value = mask.split(match[0])[0];
          "year" === targetProp ? (dateObj[targetProp] = extendYear(value), dateObj["raw" + targetProp] = value) : dateObj[targetProp] = value.replace(/[^0-9]/g, "0"), mask = mask.slice((value + match[0]).length), targetProp = void 0;
        }
      }return void 0 !== targetProp && ("year" === targetProp ? (dateObj[targetProp] = extendYear(mask), dateObj["raw" + targetProp] = mask) : dateObj[targetProp] = mask.replace(/[^0-9]/g, "0")), dateObj;
    }
    var tokenizer,
        formatCode = {
      d: "[1-9]|[12][0-9]|3[01]",
    ...
    return Inputmask.extendAliases({
      datetime: {
        mask: function mask(opts) {
          return opts.inputFormat = formatAlias[opts.inputFormat] || opts.inputFormat, opts.displayFormat = formatAlias[opts.displayFormat] || opts.displayFormat || opts.inputFormat, opts.outputFormat = formatAlias[opts.outputFormat] || opts.outputFormat || opts.inputFormat, opts.placeholder = opts.placeholder || opts.inputFormat, opts.regex = parse(opts.inputFormat), null;
        },
        ...
        postValidation: function postValidation(buffer, currentResult, opts) {
          ...
          if (result && isFinite(dateParts.rawyear) && (result = result && ("29" !== dateParts.day || !isLeapYear(dateParts.rawyear))), result) {
        ...
        onKeyDown: function onKeyDown(e, buffer, caretPos, opts) {
          ...
          if (e.ctrlKey && e.keyCode === Inputmask.keyCode.RIGHT) {
            for (var match, today = new Date(), date = ""; match = getTokenizer().exec(opts.inputFormat);) {
              "d" === match[0].charAt(0) ? date += today.getDate().toString() : "m" === match[0].charAt(0) ? date += today.getMonth().toString() : "yyyy" === match[0] ? date += today.getFullYear().toString() : "yy" === match[0] && (date += today.getYear().toString());
  ...
  __webpack_require__(8), __webpack_require__(3), __webpack_require__(4), __webpack_require__(5), __webpack_require__(6);
...
(function (factory) {
  ...
  // ease-in-out-cubic
  $.easing.donutProgressEasing = function (x) {
    if (x < 0.5) {
      x = 2 * x;
      return 0.5 * x * x * x;
    } else {
   ...
   *
   * ```js
   * el.donutProgress('widget'); // get the <canvas>
   * el.donutProgress('value'); // get the value
   * el.donutProgress('value', newValue); // update the value
   * el.donutProgress('redraw'); // redraw the circle
   * el.donutProgress(); // the same as 'redraw'
   * ```
   *
   ...
   * @alias "$(...).donutProgress"
   */
  $.fn.donutProgress = function (configOrCommand, commandArgument) {
...
(function ($) {
  ...
  $('.first.donut').donutProgress({
    value: 1,
    animation: false,
    fill: '#2688eb'
  });

  $('.second.donut').donutProgress({
    value: .5,
    animation: false,
    fill: '#2688eb'
  });
  ...
  $('.forth.donut').donutProgress({
    value: 1,
    fill: { gradient: [['#2688eb', .5], ['#f42c2c', .5]] }
  });

  $('.fifth.donut').donutProgress({
    value: .5,
    fill: '#f42c2c'
  });

  $('.six.donut').donutProgress({
    value: .0,
    fill: '#f42c2c'
  });
})(jQuery);