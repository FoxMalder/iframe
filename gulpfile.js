  ...
  var src = 'app/',
      ...
      paths = {
        ...
        watch: {
          ...
          stylesskin:  [src + 'styles/layout-skin.pcss'],
          images:      [src + 'images/**/*.*'],
          resources:   [src + 'resources/**/*.*']
        }
  };
...
/*---------- Tasks ----------*/
  ...
  // Компиляция стилей для темизации
  gulp.task('styles:skin', function () {
      return gulp.src(paths.source.styles + 'layout-skin.pcss')
        .pipe(plumber(plugins.plumber))
        .pipe(changed(paths.build.styles))
        .pipe(postcss(processors))
...
/*---------- Бдительные вотчеры ----------*/
  ...
  gulp.task('watch', function() {
    ...
    watch(paths.watch.stylesskin, function() {
        return runSequence('styles:skin', browserSync.reload);
    });
...
/*---------- Режимы ----------*/
  ...
  gulp.task('default', function(cb) {
    return runSequence(
      'copy',
      ['html', 'styles', 'styles:skin', 'scripts'],
      'watch',
      'server',
      cb
    );
